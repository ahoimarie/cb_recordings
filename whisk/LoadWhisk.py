...
Chen, S., Augustine, G. J., & Chadderton, P. (2016). The cerebellum linearly encodes whisker position during voluntary movement. ELife, 1–16. https://doi.org/10.7554/eLife.10509

Whisking parameter extraction is based on the script accompanies the Primer "Analysis of Neuronal Spike Trains, Deconstructed", by J. Aljadeff, B.J. Lansdell, A.L. Fairhall and D. Kleinfeld (2016) Neuron, 91 link to manuscript: http://dx.doi.org/10.1016/j.neuron.2016.05.039

This script takes as input the filepath to the whisking data.
The input data consist of traces were generated by Nathan Clack's automated tracking of single rows of whiskers in high-speed video Janelia https://wiki.janelia.org/wiki/display/MyersLab/Whisker+Tracking
The (huge) output files from the automated tracking were processed in Matlab using Janelia's available code and converted to .npy using custom matlab function conv_meas_py.m available on github. The .npy contain three columns [FID WhiskerLabels Angles], where FID corresponds to frame ID, whisker labels to the whisker ID (0, 1, 2, ...) and the Angles to the measured angle of traced whiskers in degrees. Angles are wrapped to 0-360 degrees. FIDs and Angles are linearly interpolated for missing frames. This interpolation can be improved.
...
def loadWhiskerData(filepath):
    ...
    for j in np.unique(labels):
        ...
        # ampfilt = filtfilt(a,[1,a-1],amp,padtype = 'odd', padlen=3*(max(np.size(a),len([1,a-1]))-1)) # almost identical to Aljadeffs ver
